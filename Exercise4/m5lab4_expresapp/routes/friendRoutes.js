...
// 2. Get information about this request from the headers (do I have to change info for headers???? or what do I have to modify )
...
router.put("/:id", (req, res) => {
  ...
  let findIndex = friends.findIndex((friend) => friend.id === friendId);

  if (findIndex === -1) {
    res.status(404).send("frind can not be found");
    return;
  }
  friends[findIndex] = updatedFriend;
  res.json({
